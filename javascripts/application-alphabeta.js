...
var computer = {
  ...
  startTurn: function() {
    if (game.currentTurn() == computer && !game.grid.isFull() && !game.grid.hasWinner()) {
      setTimeout(function() {
        var move = computer.chooseMove(game.turn, -1, 1);
        game.markCell(move.index);
        game.toggleTurn();
      });
    }
  },
  chooseMove: function(turn, alpha, beta) {
    var grid = game.grid;
    ...
    if (grid.isFull() || grid.hasWinner()) {
      ...
      } else if (game.currentTurn() == human) {
        bestMove.score = beta;
      }

      grid.freeCells().forEach(function(cell) {
        grid.mark(cell, game.players[turn].mark);
        var move = computer.chooseMove((turn + 1) % 2, alpha, beta);
        move.index = cell;
        grid.unmark(cell);
        ...
        if (bestMove.index == null) {
          ...
          } else if (game.players[turn] == human) {
            beta = bestMove.score;
          }
        } else if (game.players[turn] == computer && move.score > bestMove.score) {
          bestMove = move;
          alpha = bestMove.score;
        } else if (game.players[turn] == human && move.score < bestMove.score) {
          bestMove = move;
          beta = bestMove.score;
        }
